...
the $p_k$'s, where $1\leq k \keq K$, with $K$ being the assumed maximum number of offspring.
...
likelihood <- function(data, p) {
  multi <- (log(dmultinom(data, prob = p)))
  return(multi)
}
...
mcmc_multinomial_dirichlet <- function(N, alpha, data) {
  p_list <- list()
  p_list[[1]] <- c(1/6, 1/6, 1/6, 1/6, 1/6, 1/6)
  
  for (i in 1:N) {  # Start loop from i = 1

    current <- p_list[[i]]
    proposal <- gtools::rdirichlet(1, c(1,1,1,1,1,1))
...
new_x <- X/155
new_x
# alpha <- c(4,4,4,4,2,2,2,2,1,1,1,1)
alpha <- c(1,1,1,1,1,1)
